...
* **离线 HTML 网页 (.html)**：带苹果风卡片式排版。内置一个响应式交互的 **SVG 矢量网络图谱**，支持无损缩放、局部高保真打印和原生 CSS 过渡动画。通过在页面中嵌入轻量级 JS 脚本监听 DOM 鼠标事件，实现了极速流畅的节点悬浮高亮、关联连线发光及动态卡片式 Tooltip。
...
| **Canvas 命中及悬停检测 (交互优化)** | $O(\log N)$ | $O(N)$ | Canvas 交互采用 `d3.quadtree` 空间分割树。每帧绘制时构建树需要 $O(N \log N)$ 时间与 $O(N)$ 空间，而单次悬停、拖拽或点击检测仅需 $O(\log N)$ 时间。 |

---

## 🧪 八、边界情况与测试设计

系统在 Python 单元测试（pytest 23个通过）和 Vitest（70个通过）中对图底层进行了多层断言测试，保证健壮性：
...
## ⚡ 十、系统工程化与性能升级 (IndexedDB & Quadtree Canvas)

### 1. 底层存储层迁移：IndexedDB 异步存储
* **设计初衷**：当自定义节点、活动注册和学生画像的数据不断扩充时，传统的同步 `localStorage` 极易溢出 5MB 的存储限制，且同步读写大 JSON 会导致浏览器主线程阻塞，造成画面瞬间卡顿。
* **技术实现**：封装了 Promise 风格的 `IndexedDBHelper` 并实例化为全局数据库单例 `graphDb`，将所有大容量、结构化的网络拓扑和画像注册信息迁移至 IndexedDB。在 Node.js 测试环境（Vitest）中，工具会自动启用在内存中维护的同步 Map 存储作为 fallback，完美实现了开发环境的低开销单测与生产环境的异步持久化。

### 2. Canvas 交互渲染优化：四叉树 (d3.quadtree) 空间分割索引
* **性能痛点**：在 Canvas 社交网络关系图中，用户移动鼠标触发的命中检测（确定鼠标当前选中的节点）默认采用 D3 提供的 `simulation.find()`。这是一种 **$O(N)$ 线性扫描算法**。当高频触发的 `mousemove` 事件以每秒数十次频率运行时，每次均需遍历全图计算距离平方。节点规模增至几百个以上时，就会引发明显的微小卡顿（Stuttering）。
* **技术实现**：在每帧 Canvas 绘制的 tick 流程中，利用最新位置的节点坐标一键构建 `d3.quadtree`。交互层（包括拖拽抓取、缩放排布、悬停 Tooltip 诊断、点击选中等）在触发命中检测时，会首先查询四叉树 `quadtree.find(simX, simY, radius)`。四叉树利用其空间划分的特性，以 **$O(\log N)$ 的时间复杂度**快速定位最近点，避开了无关区域的大量距离公式计算，确保了数千级节点下 60FPS 的极致流畅交互体验。
